...
def _footer(slide, n, presenter=""):
    bar = _rect(slide, 0, 0, SW, Inches(0.16), NAVY)  # top accent bar
    acc = _rect(slide, 0, 0, Inches(3.2), Inches(0.16), ACCENT)
...
def add_title(title, subtitle, meta, presenters):
    ...
    for i, line in enumerate(meta):
        ...
        pp.space_after = Pt(2)
    pp = tm.add_paragraph()
    rr = pp.add_run(); rr.text = presenters
...
def add_divider(part, title, subtitle, presenter, n):
    ...
    rk = tk.paragraphs[0].add_run(); rk.text = part
    _set(rk, 15, ACCENT, bold=True)
    ...
    rp = tp.paragraphs[0].add_run(); rp.text = f"Trình bày: {presenter}"
...
def _render_bullets(tf, items):
    ...
    for it in items:
        ...
        if kind in LABELS:
            ...
            dot = p.add_run(); dot.text = "● "; _set(dot, 13, col, bold=True)
            lab = p.add_run(); lab.text = f"{label}: "; _set(lab, 14.5, col, bold=True)
            txt = p.add_run(); txt.text = it[1]; _set(txt, 14.5, DARK)
        elif kind == "p":
            dot = p.add_run(); dot.text = "▪ "; _set(dot, 12, NAVY, bold=True)
            txt = p.add_run(); txt.text = it[1]; _set(txt, 14.5, DARK)
        elif kind == "s":
            p.level = 1
            dot = p.add_run(); dot.text = "– "; _set(dot, 12, GREY)
            txt = p.add_run(); txt.text = it[1]; _set(txt, 12.5, GREY)
            p.space_after = Pt(4)
        elif kind == "key":
            bar = p.add_run(); bar.text = "▶ Số liệu chốt:  "; _set(bar, 13.5, ACCENT, bold=True)
            txt = p.add_run(); txt.text = it[1]; _set(txt, 13.5, NAVY, bold=True)
            p.space_before = Pt(6)
        elif kind == "note":
            txt = p.add_run(); txt.text = it[1]; _set(txt, 12, GREY, italic=True)
            p.space_before = Pt(4)
...
def _add_table(s, headers, rows, left, top, width, col_w=None, fontsize=11):
    ...
    for j, htext in enumerate(headers):
        ...
        r = para.add_run(); r.text = htext; _set(r, fontsize, WHITE, bold=True)
...
def add_content(n, kicker, title, items, notes="", presenter="", image=None,
                caption="", table=None, text_w=12.2):
    s = prs.slides.add_slide(BLANK)
    ...
    if table:
        _add_table(s, table["headers"], table["rows"],
                   Inches(table.get("left", 0.6)), Inches(table["top"]),
                   Inches(table.get("width", 9.5)),
                   col_w=table.get("col_w"), fontsize=table.get("fs", 11))
    _notes(s, notes)
    return s
...
add_content(
    3, "BỨC TRANH TỔNG THỂ", "Pipeline dữ liệu của nhóm",
    [
        ("do", "7 bảng OULAD thô → gộp → master_raw (32.593 × 33) → cắt theo 6 mốc thời gian → tiền xử lý chống rò rỉ → train/test."),
        ("why", "Slide 'neo' để luôn biết đang ở bước nào của quy trình."),
        ("out", "6 bộ dữ liệu dataset_t10…t100 + tập test 20% cố định; 16/16 kiểm thử rò rỉ ĐẠT."),
    ],
    notes="Đây là bức tranh tổng thể. Chúng em bắt đầu từ 7 bảng OULAD thô, gộp thành một bảng "
          ...
          "một tập kiểm tra cố định; toàn bộ được 16 kiểm thử tự động xác nhận không rò rỉ.",
    image=(FIG / "preprocessing_sequence.png"),
    caption="Trình tự pipeline tiền xử lý (preprocessing_sequence.png)",
)

# ── PHẦN 1 ───────────────────────────────────────────────────────────────
add_divider("PHẦN 1", "Xác định dữ liệu cần có",
            "Mục tiêu · Biến mục tiêu · Nhóm dữ liệu · Đơn vị & phạm vi", "Sơn", 4)

add_content(
    4, "PHẦN 1 · XÁC ĐỊNH DỮ LIỆU", "Bài toán & mục tiêu đề tài", [
        ("do", "Phát hiện SỚM sinh viên nguy cơ học kém trên VLE, và GIẢI THÍCH được mỗi dự đoán."),
        ("why", "Ba khoảng trống của nghiên cứu trước: hộp đen (opacity), dự đoán muộn (lateness), mất cân bằng lớp."),
        ("out", "Dữ liệu phải phục vụ RQ1 (dự đoán sớm), RQ2 (ổn định giải thích), RQ3 (mất cân bằng)."),
    ],
    presenter="Sơn",
...
add_content(
    5, "PHẦN 1 · XÁC ĐỊNH DỮ LIỆU", "Biến mục tiêu (at_risk)", [
        ("do", "Phân loại nhị phân: at_risk suy ra từ final_result."),
        ("p", "at-risk (1) = Fail + Withdrawn  ·  not-at-risk (0) = Pass + Distinction."),
        ("why", "Gom theo 'có cần can thiệp hay không' — đúng mục tiêu cảnh báo sớm."),
        ("key", "at-risk 52,8% (17.208) vs not-at-risk 47,2% (15.385) — số THẬT, không phải 68/32."),
    ],
    presenter="Sơn",
...
add_content(
    6, "PHẦN 1 · XÁC ĐỊNH DỮ LIỆU", "Ba nhóm dữ liệu đầu vào & vai trò", [
        ("do", "Ba nhóm đặc trưng:"),
        ("s", "Nhân khẩu học — gender, region, education, imd_band, age_band, disability…"),
        ("s", "Tương tác/VLE — clickstream: tổng/loại click, số ngày hoạt động…"),
        ("s", "Kết quả đánh giá — điểm bài nộp, số bài nộp, cờ chưa nộp."),
        ("why", "Nhân khẩu = bối cảnh & công bằng; tương tác = tín hiệu hành vi sớm; kết quả = bằng chứng năng lực."),
        ("out", "Bộ đặc trưng chia 3 nhóm rõ ràng → thuận tiện diễn giải SHAP/LIME theo nhóm."),
    ],
    presenter="Sơn",
...
add_content(
    7, "PHẦN 1 · XÁC ĐỊNH DỮ LIỆU", "Đơn vị quan sát, phạm vi & dữ liệu loại trừ", [
        ("do", "Đơn vị quan sát: 1 dòng = 1 sinh viên–môn–kỳ (code_module × code_presentation × id_student)."),
        ("do", "Phạm vi: 32.593 bản ghi · 22 môn–kỳ · 7 bảng · các kỳ 2013–2014 của Open University."),
        ("do", "Loại trừ: bỏ cột định danh; KHÔNG dùng date_unregistration làm đặc trưng; bỏ final_result gốc."),
        ("why", "Tách đơn vị quan sát & loại trừ sớm để tránh rò rỉ và nhân bản dữ liệu."),
    ],
    presenter="Sơn",
...
# ── PHẦN 2 ───────────────────────────────────────────────────────────────
add_divider("PHẦN 2", "Phương pháp thu thập dữ liệu",
            "Nguồn · Cấu trúc 7 bảng · Liên kết · Ưu/Nhược/Rủi ro", "Phúc", 8)

add_content(
    8, "PHẦN 2 · THU THẬP DỮ LIỆU", "Nguồn & loại dữ liệu", [
        ("do", "Nguồn: Open University Learning Analytics Dataset (OULAD), Kuzilek và cộng sự (2017)."),
        ("do", "Loại: dữ liệu THỨ CẤP công khai, đã ẩn danh tại nguồn, giấy phép CC-BY 4.0."),
        ("why", "Đáp ứng đạo đức bằng trích dẫn đúng quy cách; không xử lý dữ liệu cá nhân nhạy cảm."),
        ("out", "Hợp pháp, tái lập, không cần thu thập sơ cấp — phù hợp môi trường học thuật."),
    ],
    presenter="Phúc",
...
add_content(
    9, "PHẦN 2 · THU THẬP DỮ LIỆU", "Cách thu thập & cấu trúc 7 bảng", [
        ("do", "Tải 7 CSV → đặt vào data/raw/ → xác minh toàn vẹn bằng MD5 (data_manifest.txt)."),
        ("do", "7 bảng: studentInfo, courses, studentRegistration, studentVle (10.655.280 dòng), vle, assessments, studentAssessment."),
        ("why", "Tách bảng nền khỏi bảng nặng → xử lý theo chunk (500k dòng/lần) tránh tràn bộ nhớ."),
        ("key", "Dữ liệu thô được 'đóng băng' + có checksum → mọi thành viên dùng đúng một bản."),
    ],
    presenter="Phúc",
...
add_content(
    10, "PHẦN 2 · THU THẬP DỮ LIỆU", "Mối liên hệ giữa các bảng", [
        ("do", "Khoá liên kết chính:"),
        ("s", "(code_module, code_presentation, id_student) — nối studentInfo ↔ registration ↔ VLE-agg ↔ performance."),
        ("s", "id_site — nối studentVle ↔ vle (lấy loại hoạt động)."),
        ("s", "id_assessment — nối studentAssessment ↔ assessments (lấy trọng số/hạn nộp)."),
        ("why", "Hiểu khoá là điều kiện để gộp KHÔNG nhân bản (validate='many_to_one')."),
...
add_content(
    11, "PHẦN 2 · THU THẬP DỮ LIỆU", "Đánh giá nguồn: phù hợp · ưu · hạn chế · rủi ro", [
        ("do", "Phù hợp: đủ 3 nhóm đặc trưng + dấu thời gian → cho phép dự đoán time-aware; 32k đủ lớn."),
        ("do", "Ưu điểm: chuẩn benchmark quốc tế, ẩn danh, tái lập, miễn phí."),
        ("why", "Hạn chế: thứ cấp 1 trường; click chỉ là số đếm (không nội dung); kỳ 2013–2014."),
        ("out", "Rủi ro: mất cân bằng nhẹ · thiếu cấu trúc · lệch phải mạnh · đa cộng tuyến · rò rỉ thời gian — đều có phương án ở Phần 3–5."),
    ],
    presenter="Phúc",
...
# ── PHẦN 3 ───────────────────────────────────────────────────────────────
add_divider("PHẦN 3", "Làm sạch dữ liệu",
            "Thiếu · Trùng · Kiểu · Ngoại lai · Gộp bảng", "Bình", 12)

add_content(
    12, "PHẦN 3 · LÀM SẠCH", "Kiểm tra tổng quan dữ liệu thô", [
...
add_content(
    13, "PHẦN 3 · LÀM SẠCH", "Dữ liệu thiếu: phát hiện & phương án", [
        ("do", "Chỉ 3 cột thiếu; điểm/số bài thiếu do chưa nộp → điền 0 + cờ not_submitted."),
        ("why", "Mỗi loại thiếu (MCAR/MAR/MNAR) xử lý khác nhau; thiếu do 'chưa nộp' là TÍN HIỆU, không phải nhiễu."),
    ...
    table={"headers": ["Cột", "Số thiếu", "Xử lý"],
           "rows": [["date_unregistration", "22.521", "Bỏ (thiếu cấu trúc)"],
                    ["imd_band", "1.111", "Điền 'Unknown'"],
                    ["date_registration", "45", "Trung vị train"]],
           "top": 4.55, "width": 6.9, "left": 0.6,
           "col_w": [Inches(3.0), Inches(1.5), Inches(2.4)], "fs": 11},
    image=(FIG / "quality_missingness.png"),
    caption="quality_missingness.png",
)

add_content(
    14, "PHẦN 3 · LÀM SẠCH", "Trùng lặp & chuẩn hoá định dạng", [
        ("do", "Trùng: kiểm tra theo khoá sinh viên–môn–kỳ → drop_duplicates → 0 khoá trùng."),
        ("do", "Kiểu dữ liệu: danh mục numeric / ordinal / nominal / binary / indicator (VARIABLE_TYPES)."),
        ("do", "Chuẩn hoá: strip text; đối chiếu từ điển (region 13, education 5, imd_band 10, age_band 3, gender/disability 2)."),
        ("why", "Nhận diện đúng kiểu là điều kiện mã hoá đúng ở Phần 4; chuẩn hoá để 'Y ' ≠ 'Y' không xảy ra."),
...
add_content(
    15, "PHẦN 3 · LÀM SẠCH", "Giá trị bất thường & ngoại lai: phát hiện", [
        ("do", "Dò bất thường logic (điểm ∈ [0–100]) + dò ngoại lai bằng quy tắc IQR (log_outliers)."),
        ("why", "Clickstream lệch phải rất mạnh: clicks_resource skew ≈ 35, kurtosis ≈ 2.125; max_clicks_single_day max = 6.988."),
        ("out", "Bảng 7 biến nghi ngoại lai — bàn giao Bình → Đức (handoff_outliers_for_Duc.csv)."),
    ],
    presenter="Bình",
...
add_content(
    16, "PHẦN 3 · LÀM SẠCH", "Xử lý bất thường & ngoại lai", [
        ("do", "Chiến lược theo biến: log1p cho 12 biến click lệch mạnh · winsorize (cắt 1% hai đầu) cho biến lệch vừa · none cho biến trong phạm vi tự nhiên."),
        ("why", "KHÔNG loại bỏ dòng nào: giá trị cực trị của sinh viên at-risk (vd học lại nhiều lần) là tín hiệu cần giữ."),
...
add_content(
    17, "PHẦN 3 · LÀM SẠCH", "Gộp bảng & bộ dữ liệu sạch cuối cùng", [
        ("do", "Gộp: studentInfo (nền) ← left join ← registration ← engagement ← performance ← courses."),
        ("why", "Nhật ký trước/sau chứng minh không nhân bản, không thất thoát (validate='many_to_one')."),
        ("out", "master_raw = 32.593 × 33 — bộ dữ liệu sạch, một dòng một sinh viên–môn–kỳ."),
    ],
    presenter="Bình",
    ...
    table={"headers": ["Bước", "rows_before", "rows_after"],
           "rows": [["studentInfo (nền)", "32.593", "32.593"],
                    ["+ registration", "32.593", "32.593"],
                    ["+ engagement", "32.593", "32.593"],
                    ["+ performance", "32.593", "32.593"]],
           "top": 4.6, "width": 9.0,
           "col_w": [Inches(4.2), Inches(2.4), Inches(2.4)], "fs": 12},
)

# ── KHỐI EDA ─────────────────────────────────────────────────────────────
add_divider("KHỐI EDA", "Phân tích khám phá dữ liệu",
            "Hiểu dữ liệu · Kiểm tra chất lượng · Tìm tín hiệu phân biệt", "Bình", 18)

add_content(
    18, "EDA · MỤC TIÊU", "Mục tiêu EDA", [
        ("p", "Hiểu dữ liệu: 32.593 sinh viên, 33 biến (nhân khẩu học · tương tác · điểm số)."),
...
add_content(
    19, "EDA · UNIVARIATE", "Phân tích phân phối (Univariate)", [
        ...
        ("key", "clicks_resource skew 34,71 · kurtosis 2.125,45 → lệch phải, đuôi nặng."),
    ],
    presenter="Bình",
...
add_content(
    20, "EDA · BIVARIATE", "Phân tích theo nhãn at-risk (Bivariate)", [
        ...
        ("p", "Top biến: days_since_last_activity d=2,55 · n_assessments_submitted 2,05 · weighted_score_to_date 1,96."),
...
add_content(
    21, "EDA · MULTIVARIATE", "Tương quan & kiểm tra rò rỉ (Multivariate)", [
        ("p", "Heatmap Pearson (tuyến tính) + Spearman (đơn điệu, hợp dữ liệu lệch)."),
        ("p", "Tương quan với nhãn: days_since +0,78 · n_assessments −0,72 · weighted_score −0,71."),
...
add_content(
    22, "EDA · TIME-AWARE (RQ1)", "Phân tích theo thời gian & kết luận EDA", [
        ("p", "Đo Cohen's d từng biến tại 6 mốc 10%→100%; tín hiệu mạnh dần đều."),
        ("p", "Mốc đầu đạt d ≥ 0,8: n_days_active từ 10% · điểm/số bài từ 20% · days_since từ 40%."),
        ("p", "Withdrawn ngừng hoạt động sớm: median 233 ngày & 89 click vs 11 ngày & 1.425 click."),
        ("out", "Dự đoán sớm khả thi từ ~20% khóa học; ưu tiên đặc trưng tương tác + điểm."),
        ("key", "weighted_score d: 0,61 → 1,96 · n_assessments: 0,67 → 2,05 (10%→100%)."),
    ],
    presenter="Bình",
...
# ── PHẦN 4 ───────────────────────────────────────────────────────────────
add_divider("PHẦN 4", "Chuẩn hoá & biến đổi dữ liệu",
            "Mã hoá · log1p · Chuẩn hoá · Đặc trưng · Mất cân bằng", "Đức", 23)

add_content(
    23, "PHẦN 4 · CHUẨN HOÁ & BIẾN ĐỔI", "Mã hoá biến phân loại", [
    ...
    table={"headers": ["Loại", "Biến", "Phương pháp"],
           "rows": [["Thứ bậc", "highest_education, imd_band, age_band", "OrdinalEncoder"],
           ...
           "top": 3.9, "width": 12.1,
           "col_w": [Inches(2.0), Inches(6.6), Inches(3.5)], "fs": 12},
)

add_content(
    24, "PHẦN 4 · CHUẨN HOÁ & BIẾN ĐỔI", "Biến đổi phân phối lệch (log1p)", [
        ("do", "Áp log1p cho các biến click lệch phải mạnh (đã nêu ở Phần 3)."),
        ("why", "Giảm đuôi nặng → ổn định mô hình tuyến tính & khoảng cách; bằng chứng skew ~13–35 ở EDA."),
...
add_content(
    25, "PHẦN 4 · CHUẨN HOÁ & BIẾN ĐỔI", "Chuẩn hoá thang đo (StandardScaler)", [
        ("do", "StandardScaler đưa biến số về trung bình 0, độ lệch chuẩn 1."),
        ("why", "Tổng click (0–hàng nghìn) áp đảo điểm số (0–100) → bắt buộc cho Logistic Regression & ANN."),
        ("out", "CHỈ fit trên train, transform cho cả train/test (in scaler.mean_ chỉ tính từ train)."),
    ],
    presenter="Đức",
...
add_content(
    26, "PHẦN 4 · CHUẨN HOÁ & BIẾN ĐỔI", "Tạo đặc trưng mới & kiểm tra ý nghĩa", [
        ("do", "Tương tác: total_clicks, n_days_active, 8× clicks_<loại>, max/mean clicks, days_since_last_activity."),
        ("do", "Kết quả: mean_score_to_date, weighted_score_to_date, n_assessments_submitted, cờ not_submitted."),
        ("why", "Mỗi đặc trưng gắn giả thuyết: 'ngừng hoạt động lâu / bỏ nộp bài ⇒ nguy cơ cao'."),
        ("out", "Kiểm tra ý nghĩa bằng |Cohen's d|: days_since 2,55 · n_assessments 2,05 · weighted_score 1,96."),
    ],
    presenter="Đức",
...
add_content(
    27, "PHẦN 4 · CHUẨN HOÁ & BIẾN ĐỔI", "Mất cân bằng dữ liệu & phương án", [
        ("do", "Kiểm tra: at-risk 52,8% / not-at-risk 47,2% → tỷ số mất cân bằng 1,12 (nhẹ)."),
        ("why", "Bỏ sót sinh viên nguy cơ là sai lầm đắt nhất → chỉ số chính là PR-AUC & recall lớp at-risk."),
        ("out", "Phương án RQ3: so sánh no-resampling / class-weight / SMOTE / ADASYN; CHỈ tái lấy mẫu trên train."),
    ],
    presenter="Đức",
...
# ── PHẦN 5 ───────────────────────────────────────────────────────────────
add_divider("PHẦN 5", "Tách train/test & chống rò rỉ",
            "Tỷ lệ · Phân tầng + nhóm · Time-aware · Lưu & kiểm tra", "Khoa", 28)

add_content(
    28, "PHẦN 5 · TÁCH & CHỐNG RÒ RỈ", "Mục đích, tỷ lệ & phương pháp chia", [
        ("do", "Tỷ lệ 80% train / 20% test (TEST_SIZE = 0.2)."),
        ("do", "Phương pháp: StratifiedGroupKFold — phân tầng theo at_risk + gom nhóm theo id_student."),
        ("why", "Một SV học nhiều môn–kỳ ⇒ gom theo id_student để KHÔNG cho cùng người ở cả train lẫn test (rò rỉ nhóm)."),
        ("out", "Test ≈ 6.489 dòng / 6,5k sinh viên · train ≈ 26.104 dòng."),
    ],
    presenter="Khoa",
...
add_content(
    29, "PHẦN 5 · TÁCH & CHỐNG RÒ RỈ", "Kiểm soát rò rỉ: 2 trục", [
        ("do", "Trục thời gian: cut_at_checkpoint() chỉ giữ sự kiện ≤ ngày mốc; cutoff_day = round(length × t/100)."),
        ("do", "Trục đặc trưng: mọi bộ học (encoder, scaler, resampling) CHỈ fit trên train."),
        ("why", "Mô phỏng đúng 'thông tin có tại thời điểm dự đoán' — không nhìn tương lai."),
        ("out", "6 bộ dataset_t10…t100, cùng danh sách sinh viên; nhãn cố định 52,8% qua các mốc."),
    ],
    presenter="Khoa",
...
add_content(
    30, "PHẦN 5 · TÁCH & CHỐNG RÒ RỈ", "Trình tự tiền xử lý đúng & áp dụng train→test", [
        ("do", "Trình tự: chia train/test → điền khuyết → ngoại lai → mã hoá/chuẩn hoá → tái lấy mẫu."),
        ("do", "Tham số (median, ngưỡng winsorize, mean/std, danh mục encoder) học từ train → transform lên test."),
        ("why", "Fit trên toàn bộ dữ liệu trước khi chia ⇒ test 'rò rỉ' vào train ⇒ kết quả ảo."),
        ("out", "Phân bố nhãn sau chia: train 0,53 / test 0,52 — chênh ≤ 0,02, đạt phân tầng."),
    ],
    presenter="Khoa",
...
add_content(
    31, "PHẦN 5 · TÁCH & CHỐNG RÒ RỈ", "Lưu & kiểm tra lại: rò rỉ đã bị chặn", [
        ("do", "Lưu: test_student_ids.csv (commit vào repo) + parquet train/test; split_report.csv."),
        ("do", "Kiểm tra lại: 0 sinh viên trùng giữa train/test; không lệch kiểu/cột; nhãn cân đối."),
        ("why", "Test cố định, dùng lại y hệt qua cả 6 mốc ⇒ 6 điểm hiệu năng SO SÁNH ĐƯỢC."),
        ("key", "tests/test_leakage.py: 16/16 kiểm thử ĐẠT — bằng chứng tự động không rò rỉ."),
    ],
    presenter="Khoa",
    notes="Định nghĩa tập test được lưu cố định và commit vào repo, kèm báo cáo split_report. Kiểm tra "
          ...
          "điểm hiệu năng so sánh được với nhau. Toàn bộ được 16 kiểm thử tự động xác nhận không rò rỉ.",
)

# ── KẾT LUẬN ─────────────────────────────────────────────────────────────
add_content(
    32, "KẾT LUẬN", "Tóm tắt Task 3", [
        ("do", "Năm mục Task 3 đều hoàn thành với sản phẩm & số liệu cụ thể (xem bảng)."),
    ],
    presenter="An",
    ...
    table={"headers": ["Mục", "Kết quả của nhóm"],
           "rows": [["1. Xác định dữ liệu", "at_risk nhị phân 52,8%; 3 nhóm; 32.593 sv–môn–kỳ"],
                    ["2. Thu thập", "OULAD CC-BY 4.0; 7 bảng; 10,6 triệu click; có MD5"],
                    ["3. Làm sạch", "0 trùng; log1p+winsorize, 0 dòng bị xoá; master 32.593×33"],
                    ["4. Chuẩn hoá & biến đổi", "Ordinal+OneHot+Binary; StandardScaler; d≤2,55; RQ3"],
                    ["5. Tách train/test", "80/20 group+stratified; 6 mốc; 16/16 test đạt"]],
           "top": 2.7, "width": 12.2,
           "col_w": [Inches(3.3), Inches(8.9)], "fs": 12.5},
)

s33 = add_content(
    33, "KẾT LUẬN", "Đầu ra & bước tiếp theo", [
        ("out", "Đầu ra Task 3: master_raw + 6 bộ dataset_t10…t100 + tập test cố định + pipeline tái lập (seed=42, MD5, Restart & Run All)."),
        ("do", "Bước tiếp: benchmark mô hình theo từng mốc (RQ1) → xử lý mất cân bằng (RQ3) → SHAP/LIME & độ ổn định giải thích (RQ2)."),
