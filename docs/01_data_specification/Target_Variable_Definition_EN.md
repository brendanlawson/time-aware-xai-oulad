...
**Measured consequence.** XGBoost recall on the at-risk class at *t* = 40/80/100% is 0.81/0.90/0.93 on the full cohort but 0.678/0.783/0.853 on the still-enrolled subgroup (`reports/tables/sensitivity_active_xgb.csv`, produced by `tools/sensitivity_active.py`). Against the recall ≥ 0.80 criterion of RQ1, the full cohort qualifies from *t* = 40%, whereas the still-enrolled subgroup qualifies only at *t* = 100%. Consistently, the strongest SHAP feature is `days_since_last_activity` (mean |SHAP| 3.66): on the full cohort, part of the model's strength is *detecting students who have already left*, not only *forecasting future risk*. This is **not leakage** — the label never enters the features, and the low activity of withdrawn students is genuine behaviour — it is a **population-definition issue**.
