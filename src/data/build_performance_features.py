...
Supports Task 4 (master table) and Task 13 (checkpoints). Per student-module-
presentation, computed *as of a cutoff day*:
    ...
    not_submitted             - 1 if the student missed >= 1 assessment whose
                                deadline already passed (deadline <= cutoff and
                                not submitted by cutoff); a known risk signal.

`aggregate_performance` is pure and cutoff-driven: the master table passes the
module length as the cutoff (t=100%), the checkpoint builder passes the
checkpoint day. `not_submitted` distinguishes a genuine miss from "no deadline
yet", so filling missing scores with 0 downstream stays meaningful.
"""
...
from src.data.io_utils import GROUP_COLS, PRESENTATION_KEY  # noqa: E402
